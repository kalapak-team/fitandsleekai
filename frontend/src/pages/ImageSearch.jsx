...
export default function ImageSearch() {
  ...
  const mapResults = (rawResults = []) =>
    rawResults.map((item, idx) => {
      const productName = item.payload?.product_name || '';
      const imagePath = item.payload?.image_url;

      return {
        id: item?.id != null ? `${item.id}-${idx}` : `item-${idx}`,
        name: productName,
        price: item.payload?.price,
        image_url: imagePath ? resolveImageUrl(imagePath) : undefined,
  ...
  const handleUrlSearch = async () => {
    ...
    try {
      console.log('🌐 [handleUrlSearch] POST http://127.0.0.1:8080/scan-to-search-url');
      const res = await axios.post('http://127.0.0.1:8080/scan-to-search-url', { url: urlInput });

      console.log('✅ [handleUrlSearch] Response:', res.data);

      const nextProducts = mapResults(res.data?.result || []);
  ...
  const sendImageToBackend = async (file) => {
    ...
    try {
      console.log('🌐 [sendImageToBackend] POST http://127.0.0.1:8080/scan-to-search');
      const response = await axios.post('http://127.0.0.1:8080/scan-to-search', formData, {
      ...
      const prods = mapResults(response.data?.result || []);

      console.log('📊 [sendImageToBackend] Results mapped:', { productCount: prods.length });
